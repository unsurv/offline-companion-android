...
// TODO when NFc is scanned, query db for current area shown, if small number of cameras in db, create toast
...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    // download button
    downloadButton = findViewById(R.id.map_download_button);

    downloadButton.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {

        BoundingBox borders = mapView.getBoundingBox();

        ConnectionUtils.downloadCamerasFromServer(borders.getLatSouth(), borders.getLatNorth(), borders.getLonWest(), borders.getLonEast(), true, cameraRepository, ctx);
  ...
  void populateMapFromNfc(JSONArray nfcJSON) {
      ...
      for (int i = 0; i < nfcJSON.length(); i++) {
        ...
        for (int k = 0; k < ids.length(); k++) {
          ...
          if (camera != null) {
            ...
            if (!camerasOnMap.contains(camera)) {
    ...
    Drawable deviceLocationMarker = ContextCompat.getDrawable(ctx, R.drawable.marker_teal);
